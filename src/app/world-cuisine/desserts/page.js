...
export default function WorldDesserts() {
    ...
    const fetchDesserts = async () => {
      try {
        ...
        const filtered = response.data.filter(r => 
          r.category === 'world-dessert' || r.category === 'Dünya Tatlıları'
        );
        
        setDesserts(filtered);
      } catch (error) {
        console.error("Dünya tatlıları yüklenemedi!", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const text = {
    ...
    header: { tr: "Dünya Tatlıları", en: "World Desserts" },
    subHeader: { 
      tr: "Global mutfaklardan hayvansal ürün içermeyen tatlı kaçamaklar.", 
      en: "Animal-product-free sweet escapes from global cuisines." 
    },
    vegan: { tr: "Vegan", en: "Vegan" }
  };

  if (loading) return <div className="text-center py-20 italic text-amber-600">Tatlılar hazırlanıyor... 🍩</div>;
    ...
    <div className="min-h-screen bg-[#FDFBF7] py-12 px-6">
      <div className="text-center mb-16">
        ...
        <span className="block text-amber-600 font-bold tracking-wider uppercase text-xs mt-2">
          {language === 'tr' ? 'Tatlı Kaçamaklar' : 'Sweet Escapes'}
        </span>
