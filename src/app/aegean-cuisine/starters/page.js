...
export default function AegeanStarters() {
    ...
    const fetchRecipes = async () => {
      try {
        ...
        const filtered = response.data.filter(r => 
          r.category === 'starter' || r.category === 'Başlangıçlar'
        );
        
        setRecipes(filtered);
      } catch (error) {
        console.error("Başlangıçlar mutfağa ulaşamadı!", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-[#FDFBF7] italic text-green-800">Mezeler hazırlanıyor... 🌿</div>;
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <span className="text-green-600 font-bold tracking-wider uppercase text-sm">
            {language === 'tr' ? 'Hafif ve Sağlıklı' : 'Light and Healthy'}
          </span>
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-green-900 mt-3 mb-6">
            {language === 'tr' ? 'Ege Başlangıçları' : 'Aegean Starters'}
          </h1>
        </div>
