...
export default function Soups() {
    ...
    const fetchRecipes = async () => {
      try {
        ...
        const filtered = response.data.filter(r => 
          r.category === 'soup' || r.category === 'Çorbalar'
        );
        
        setRecipes(filtered);
      } catch (error) {
        console.error("Çorbalar servisi yolda kaldı!", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="text-center py-20 italic text-green-800">Çorbalar kaynıyor... 👨‍🍳</div>;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <span className="text-green-600 font-bold tracking-wider uppercase text-sm">
            {language === 'tr' ? 'İçinizi Isıtanlar' : 'Warm Your Soul'}
          </span>
